...
# --- Trunk metrics ---
asterisk_pjsip_trunks_total_metric = Gauge("asterisk_pjsip_trunks_total", "Total PJSIP trunks", ["host"], registry=registry)
asterisk_pjsip_trunks_online_metric = Gauge("asterisk_pjsip_trunks_online", "Total online PJSIP trunks", ["host"], registry=registry)
asterisk_pjsip_trunks_offline_metric = Gauge("asterisk_pjsip_trunks_offline", "Total offline PJSIP trunks", ["host"], registry=registry)

asterisk_pjsip_trunk_status = Gauge(
    "asterisk_pjsip_trunk_status",
    "PJSIP trunk status (1=Avail, 0=Unavail)",
    ["host", "trunk", "ip"],
    registry=registry
)

asterisk_pjsip_trunk_rtt = Gauge(
    "asterisk_pjsip_trunk_rtt_seconds",
    "PJSIP trunk round-trip time in seconds",
    ["host", "trunk"],
    registry=registry
)
...
def debug_app(environ, start_response):
    ...
    output_aor = os.popen("asterisk -rx 'pjsip show aors'").read()
...
def parse_registrar_contacts():
    ...
    """
    DATA = os.popen("asterisk -rx 'database show registrar/contact' 2>/dev/null").read()
    results = []
...
def run_metrics_collector():
    ...
    while True:
        try:
            ...
            active_calls = get_int(" asterisk asterisk -rx 'core show calls' | grep 'active call' | awk '{print $1}'")
            calls_processed = get_int(" asterisk asterisk -rx 'core show channels' | grep 'calls processed' | awk '{print $1}'")
            core_uptime = get_int(" asterisk asterisk -rx 'core show uptime seconds' | awk '/System uptime/ {print $3}'")

            output_aor = os.popen(" asterisk asterisk -rx 'pjsip show aors' 2>/dev/null").read()
            ...
            # Tách extension vs trunk: extension = tên thuần số, trunk = có chữ cái
            ext_total = 0
            ext_online = 0
            for aor_name in total_aor_matches:
                if aor_name.startswith('<') or not re.fullmatch(r'\d+', aor_name):
                    continue  # bỏ qua header và trunk
                ext_total += 1
            for b in blocks:
                name_match = re.match(r'\s*(\S+)', b)
                if not name_match:
                    continue
                aor_name = name_match.group(1)
                if aor_name.startswith('<') or not re.fullmatch(r'\d+', aor_name):
                    continue  # bỏ qua header và trunk
                if re.search(r'Contact:.*\bAvail\b', b, re.MULTILINE):
                    ext_online += 1
            ext_offline = max(ext_total - ext_online, 0)
            ...
            asterisk_total_pjsip_aors_metric.labels(host=host).set(ext_total)
            asterisk_total_pjsip_online_metric.labels(host=host).set(ext_online)
            asterisk_total_pjsip_offline_metric.labels(host=host).set(ext_offline)

            # --- Parse trunk info từ Aor blocks (đã proven hoạt động) ---
            try:
                asterisk_pjsip_trunk_status.clear()
                asterisk_pjsip_trunk_rtt.clear()
            except Exception:
                try:
                    for s in list(asterisk_pjsip_trunk_status._metrics.keys()):
                        asterisk_pjsip_trunk_status.remove(*s)
                    for s in list(asterisk_pjsip_trunk_rtt._metrics.keys()):
                        asterisk_pjsip_trunk_rtt.remove(*s)
                except Exception:
                    logging.debug("Không thể clear() trunk metrics. Tiếp tục.")

            trunk_total = 0
            trunk_online = 0
            for block in blocks:
                # Lấy tên Aor từ dòng đầu của block
                name_match = re.match(r'\s*(\S+)', block)
                if not name_match:
                    continue
                aor_name = name_match.group(1)

                # Bỏ qua header (VD: <Aor......>) và extension (tên thuần số)
                if aor_name.startswith('<') or re.fullmatch(r'\d+', aor_name):
                    continue

                # Đây là trunk - tìm Contact line trong block
                contact_match = re.search(
                    r'Contact:.*?(\d+\.\d+\.\d+\.\d+).*?(Avail|Unavail|NonQual)\s+([\d.]+)',
                    block
                )
                if not contact_match:
                    # Trunk có trong cấu hình nhưng không có Contact (chưa kết nối)
                    trunk_total += 1
                    asterisk_pjsip_trunk_status.labels(host=host, trunk=aor_name, ip="").set(0)
                    continue

                ip = contact_match.group(1)
                status_str = contact_match.group(2)
                rtt = contact_match.group(3)
                trunk_total += 1
                is_avail = 1 if status_str == 'Avail' else 0
                if is_avail:
                    trunk_online += 1
                asterisk_pjsip_trunk_status.labels(host=host, trunk=aor_name, ip=ip).set(is_avail)
                try:
                    asterisk_pjsip_trunk_rtt.labels(host=host, trunk=aor_name).set(float(rtt))
                except ValueError:
                    pass

            trunk_offline = max(trunk_total - trunk_online, 0)
            asterisk_pjsip_trunks_total_metric.labels(host=host).set(trunk_total)
            asterisk_pjsip_trunks_online_metric.labels(host=host).set(trunk_online)
            asterisk_pjsip_trunks_offline_metric.labels(host=host).set(trunk_offline)
            logging.info("Trunks: total=%d, online=%d, offline=%d", trunk_total, trunk_online, trunk_offline)
